...
class ArticleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for post_node in post_nodes:
            # image_url是图片的地址
            imgae_url = post_node.css("img::attr(src)").extract_first("")
            post_url = post_node.css("::attr(href)").extract_first("")
            # 这里通过meta参数将图片的url传递进来，这里用parse.urljoin的好处是如果有域名我前面的response.url不生效
            # 如果没有就会把response.url和post_url做拼接
            yield Request(url=parse.urljoin(response.url, post_url),meta={"front_image_url":parse.urljoin(response.url, imgae_url)},callback=self.parse_detail)
